/**
 * anchor-cloud.js — AnchorBrowser cloud example for Oracle PeopleSoft
 *
...
const { login_peoplesoft, run_ps_query } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Oracle PeopleSoft automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_peoplesoft(page, {});
      log('Action 1 result:', data1);

      const data2 = await run_ps_query(page, {});
