# Oracle PeopleSoft Browser Automation
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Fpeoplesoft-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/peoplesoft-browser-automation)

> Automate Oracle PeopleSoft — the reliable way to interact with PeopleSoft programmatically, with or without an official API.
...
<!-- keywords: peoplesoft automation, peoplesoft browser automation, peoplesoft api alternative, psquery automation, peopletools automation, hrms automation -->

## What This Is

**Oracle PeopleSoft** (HR/ERP) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (SiteMinder / PeopleSoft SSO), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for Oracle PeopleSoft using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system requires **MFA** (Duo / RSA SecurID). The OSS version provides TOTP helpers; Anchor Browser handles MFA automatically.

## Quick Start

```bash
git clone https://github.com/Browser-Automation-Hub/peoplesoft-browser-automation.git
cd peoplesoft-browser-automation
npm install
cp .env.example .env
...
- Universities on PeopleSoft Campus Solutions
- HR data extraction without IB
- Payroll automation for on-prem PeopleSoft
- Student records data sync
...
- Oracle PeopleSoft account with appropriate permissions
...
```env
PEOPLESOFT_URL=https://your-instance.example.com/psp/ps/EMPLOYEE/HRMS/h/?tab=DEFAULT
PEOPLESOFT_USERNAME=your-username
PEOPLESOFT_PASSWORD=your-password
...
const { login_peoplesoft } = require('./src/actions');

async function main() {
  const page = await createSession();
  const result = await login_peoplesoft(page, { /* options */ });
  console.log(result);
}
...
2. **SAML SSO** (SiteMinder / PeopleSoft SSO) — intercepts the SAML redirect and completes the IdP flow
3. **MFA / TOTP** (Duo / RSA SecurID) — generates TOTP codes via `otpauth` library
4. **Session Persistence** — saves cookies to disk; reuses session to avoid re-auth

### Handling Duo / RSA SecurID MFA
...
Oracle PeopleSoft official API is limited, expensive to access, or requires complex authentication that makes programmatic integration impractical.
...
- Handles Duo / RSA SecurID MFA automatically
...
> These CSS selectors were observed in Oracle PeopleSoft web interfaces. Enterprise applications update their UIs — verify against your specific instance and submit PRs when selectors break.

> 🔍 Selector reference not yet documented for Oracle PeopleSoft. [Contribute selectors via PR](https://github.com/Browser-Automation-Hub/peoplesoft-browser-automation/pulls).
...
2. Document new selectors (they break when PeopleSoft updates its UI)
...
*Built with ❤️ for developers who need to automate Oracle PeopleSoft without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
