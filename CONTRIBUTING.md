# Contributing to Oracle PeopleSoft Browser Automation
...
- **Broken selectors**: Oracle PeopleSoft updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Oracle PeopleSoft instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.PEOPLESOFT_URL}/path/to/section`);
...
When you find working selectors for Oracle PeopleSoft, document them:

```javascript
// Oracle PeopleSoft selectors (verified YYYY-MM-DD)
...
Oracle PeopleSoft updates its UI periodically. When selectors break:
